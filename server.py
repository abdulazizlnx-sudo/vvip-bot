...
def get_roblox_id(username):
    try:
        resp = requests.post(ROBLOX_USER_API, json={"usernames": [username], "excludeBannedUsers": True}, timeout=5)
...
def give_vvip():
    ...
    vvip_data = {"passType": "VVIP", "givenBy": 0, "givenAt": int(time.time()), "timestamp": time.time(), "note": f"Given via Discord by {data.get('given_by', 'Bot')}"}
